...
class FileMonitor:
    ...
    def start(self):
        ...
        self.thread.start()
        logger = get_logger()
        logger.info(f"文件监控已启动，监控目录: {self.result_dir} 和 {self.history_dir}")
        ...
        if 'history' in abs_directory:
            for file in all_files:
                try:
                    ...
                    # 文件名格式不符合要求，跳过并记录日志
                    logger = get_logger()
                    logger.warning(f"跳过不符合命名规范的文件: {file}")
            return valid_files
        else:
        ...
        while self.running:
            try:
                ...
                if current_result_files != self.last_result_files:
                    logger = get_logger()
                    added = current_result_files - self.last_result_files
                    removed = self.last_result_files - current_result_files
                    logger.info(f"检测到result目录文件变化: 新增={added}, 删除={removed}")
                ...
                if current_history_files != self.last_history_files:
                    logger = get_logger()
                    added = current_history_files - self.last_history_files
                    removed = self.last_history_files - current_history_files
                    logger.info(f"检测到history目录文件变化: 新增={added}, 删除={removed}")
                    self.socketio.emit('files_changed', {'type': 'history'})
                    self.last_history_files = current_history_files
                    
            except Exception as e:
                logger = get_logger()
                logger.error(f"监控文件时出错: {str(e)}")
